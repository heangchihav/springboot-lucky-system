...
function Page() {
  ...
  const [managingStatusKey, setManagingStatusKey] = useState<string | null>(null)
  ...
  const handleSaveNewStatus = async () => {
    ...
    try {
      ...
      setStatuses((prev) => [...prev, { key: createdStatus.key, label: createdStatus.label }])
      setEntries((prev) => ({
        ...prev,
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/calls/statuses/${managingStatusKey}`, {
        method: 'PUT',
        headers: buildAuthHeaders(),
        credentials: 'include',
        body: JSON.stringify({
      ...
      setStatuses((prev) =>
        prev.map((status) =>
          status.key === updated.key ? { ...status, label: updated.label } : status,
        ),
      )
      closeManagePopup()
    } catch (error) {
  ...
  const handleDeleteStatus = async () => {
    ...
    try {
      ...
      setStatuses((prev) => prev.filter((status) => status.key !== managingStatusKey))
      setEntries((prev) => {
        const cloned = { ...prev }
      ...
      if (selectedStatus === managingStatusKey) {
        setSelectedStatus(statuses.find((status) => status.key !== managingStatusKey)?.key ?? '')
      }
      closeManagePopup()
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = isUpdate ? `${API_BASE_URL}/api/calls/reports/${editingReportId}` : `${API_BASE_URL}/api/calls/reports`
      ...
      if (!response.ok) {
        const errorText = await response.text()
        throw new Error(errorText || `Failed to ${isUpdate ? 'update' : 'submit'} report`)
      }

      await loadReports()
      setEntries(
        Object.fromEntries(statuses.map((status) => [status.key, 0])),
      )
      setEditingReportId(null)
    } catch (error) {
      console.error(`Failed ${editingReportId ? 'updating' : 'submitting'} report`, error)
      setApiError(`Unable to ${editingReportId ? 'update' : 'submit'} the report.`)
    } finally {
    ...
    <>
      <div className="min-h-screen px-4 py-8 relative overflow-hidden">
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-8 relative z-10">
          ...
          <section className="hidden glass-card animate-fade-in-up">
            <div className="flex items-center justify-between gap-4 mb-5">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Manage your call statuses with ease</p>
              </div>
              <button
            ...
            {showAddStatusForm && (
              <div className="mt-4 glass-card-inner animate-slide-down">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <p className="text-sm font-semibold text-white">Add New Status</p>
                </div>
                <div className="grid gap-3 md:grid-cols-2 mb-4">
                  <label className="flex flex-col gap-1.5">
                    <span className="text-xs font-medium text-slate-300">Display Name</span>
                    <input
                      type="text"
                      value={newStatusName}
                  ...
                  <label className="flex flex-col gap-1.5">
                    <span className="text-xs font-medium text-slate-300">Status Key</span>
                    <input
                      type="text"
                      value={newStatusCode}
              ...
              {statuses.map((status, index) => (
                ...
                >
                  ...
                  <input
                    ...
                    type="number"
                    onChange={(event) => handleEntryChange(status.key, event.target.value)}
          ...
          {/* Reports Table */}
          {reports.length > 0 && (
            <section className="glass-card animate-fade-in-up" style={{ animationDelay: '400ms' }}>
                  ...
                  <table className="min-w-full text-sm">
                    <thead>
                      <tr className="border-b border-white/10">
                        <th className="px-3 py-3 text-left text-[10px] uppercase tracking-wider text-slate-400 font-semibold">Created At</th>
                        <th className="px-3 py-3 text-left text-[10px] uppercase tracking-wider text-slate-400 font-semibold">Date</th>
                        <th className="px-3 py-3 text-left text-[10px] uppercase tracking-wider text-slate-400 font-semibold">Created By</th>
                        <th className="px-3 py-3 text-left text-[10px] uppercase tracking-wider text-slate-400 font-semibold">Branch</th>
                        {statuses.map((status) => (
                          <th key={status.key} className="px-3 py-3 text-left text-[10px] uppercase tracking-wider text-slate-400 font-semibold">
                            {status.label}
                          </th>
                        ))}
                        <th className="px-3 py-3 text-left text-[10px] uppercase tracking-wider text-slate-400 font-semibold">Actions</th>
                      </tr>
                    </thead>
                      ...
                      {filteredReports.map((report, index) => (
                        ...
                        >
                          ...
                          {statuses.map((status) => (
                            ...
                            >
                              {report.entries[status.key]?.toLocaleString() ?? 0}
                            </td>
                          ))}
          ...
          <div className="popup-card w-full max-w-md animate-scale-in">
            <div className="flex items-start justify-between mb-5">
              <div>
                ...
                <p className="text-[10px] text-slate-500 font-mono mt-1">{managingStatusKey}</p>
              </div>
              <button
                onClick={closeManagePopup}
