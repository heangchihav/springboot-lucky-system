...
export default function MarketingAreaBranchPage() {
  ...
  const [editingSubArea, setEditingSubArea] = useState<MarketingSubArea | null>(null)

  const [branchForm, setBranchForm] = useState(defaultBranchForm)
  const [editingBranch, setEditingBranch] = useState<MarketingBranch | null>(null)
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : 'Failed to load hierarchy', 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const upsertArea = async () => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : 'Failed to save area', 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const upsertSubArea = async () => {
    ...
    try {
      ...
      if (editingSubArea) {
        await marketingHierarchyService.updateSubArea(editingSubArea.id, payload)
      ...
      showToast(error instanceof Error ? error.message : 'Failed to save sub area', 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDelete = async (entity: 'area' | 'subArea' | 'branch', id: number) => {
    ...
    try {
      ...
      if (entity === 'subArea') await marketingHierarchyService.deleteSubArea(id)
      if (entity === 'branch') await marketingHierarchyService.deleteBranch(id)
      showToast(`${entity === 'branch' ? 'Branch' : entity === 'subArea' ? 'Sub area' : 'Area'} removed`)
      await loadAll()
    } catch (error) {
    ...
    <MarketingServiceGuard>
      <div className="space-y-8">
        <header className="space-y-2">
          <p className="text-xs uppercase tracking-[0.4em] text-amber-300/70">Marketing · Coverage</p>
          <h1 className="text-3xl font-semibold text-white">Area, sub area & branch manager</h1>
          <p className="text-sm text-slate-300">
            Create your marketing footprint. Sub areas are scoped inside areas, while branches can link to either the parent
            area or a specific sub area for more granular routing.
          </p>
        </header>
        ...
        {toast && (
          <div
            className={`rounded-2xl border px-4 py-3 text-sm ${
              toast.tone === 'success' ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-200' : 'border-red-500/30 bg-red-500/10 text-red-200'
            }`}
          >
            {toast.message}
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <section className="rounded-3xl border border-white/10 bg-white/5 p-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h2 className="text-xl font-semibold text-white">{editingArea ? 'Edit area' : 'Create area'}</h2>
              </div>
              {editingArea && (
                <button
            ...
            <div className="mt-4 space-y-4">
              ...
              <label className="text-sm text-slate-300">
                ...
                <input
                  className="mt-1 w-full rounded-2xl border border-white/10 bg-slate-900/60 px-4 py-2 text-white focus:border-amber-400/60 focus:outline-none"
                  value={areaForm.code ?? ''}
                  onChange={(event) => setAreaForm((prev) => ({ ...prev, code: event.target.value }))}
                />
              </label>
              <label className="text-sm text-slate-300">
                Description
              ...
              <label className="flex items-center gap-2 text-sm text-slate-200">
                <input
                  ...
                  onChange={(event) => setAreaForm((prev) => ({ ...prev, active: event.target.checked }))}
          ...
          <section className="rounded-3xl border border-white/10 bg-white/5 p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Sub area</p>
                <h2 className="text-xl font-semibold text-white">{editingSubArea ? 'Edit sub area' : 'Create sub area'}</h2>
              </div>
              {editingSubArea && (
                <button
                  className="text-xs text-slate-400 hover:text-white"
            ...
            <div className="mt-4 space-y-4">
              <label className="text-sm text-slate-300">
                ...
                <select
                  ...
                  onChange={(event) => setSubAreaForm((prev) => ({ ...prev, areaId: Number(event.target.value) || undefined }))}
                >
              ...
              <label className="text-sm text-slate-300">
                ...
                <input
                  ...
                  onChange={(event) => setSubAreaForm((prev) => ({ ...prev, name: event.target.value }))}
                />
              </label>
              <label className="text-sm text-slate-300">
                Code
                <input
                  ...
                  onChange={(event) => setSubAreaForm((prev) => ({ ...prev, code: event.target.value }))}
                />
              </label>
              <label className="text-sm text-slate-300">
                Description
                <textarea
                  ...
                  onChange={(event) => setSubAreaForm((prev) => ({ ...prev, description: event.target.value }))}
                />
              </label>
              <label className="flex items-center gap-2 text-sm text-slate-200">
                <input
                  ...
                  onChange={(event) => setSubAreaForm((prev) => ({ ...prev, active: event.target.checked }))}
                  className="h-4 w-4 rounded border-white/20 bg-slate-900/60 text-amber-400 focus:ring-amber-400/50"
                />
                Active
              </label>
              <button
                ...
                disabled={loading}
              >
                {loading ? 'Saving...' : editingSubArea ? 'Update sub area' : 'Add sub area'}
              </button>
            </div>
          </section>
          ...
          <section className="rounded-3xl border border-white/10 bg-white/5 p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Branch</p>
                <h2 className="text-xl font-semibold text-white">{editingBranch ? 'Edit branch' : 'Create branch'}</h2>
              </div>
              {editingBranch && (
                <button
                  className="text-xs text-slate-400 hover:text-white"
            ...
            <div className="mt-4 space-y-4">
              ...
              <label className="text-sm text-slate-300">
                ...
                <textarea
                  ...
                  onChange={(event) => setBranchForm((prev) => ({ ...prev, description: event.target.value }))}
                />
              </label>
              <label className="flex items-center gap-2 text-sm text-slate-200">
                <input
        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="text-xl font-semibold text-white">Current structure</h2>
            </div>
            <button
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-slate-950/40">
              <header className="flex items-center justify-between border-b border-white/5 px-4 py-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Areas</p>
                  <p className="text-2xl font-semibold text-white">{areas.length}</p>
                </div>
              </header>
              <div className="max-h-105 space-y-2 overflow-auto p-4 text-sm text-slate-200">
                {areas.map((area) => (
                  <article key={area.id} className="rounded-2xl border border-white/5 p-3">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-base font-semibold text-white">{area.name}</p>
                        <p className="text-xs text-slate-400">{area.code ?? '—'}</p>
                      </div>
                      <span
                        className={`rounded-full px-3 py-1 text-xs font-semibold ${
                          area.active ? 'bg-emerald-500/20 text-emerald-200' : 'bg-slate-600/40 text-slate-300'
                        }`}
                      >
                        {area.active ? 'Active' : 'Inactive'}
                      </span>
                    </div>
                    <p className="mt-2 text-xs text-slate-400">{area.description || 'No description'}</p>
                    <div className="mt-3 flex items-center gap-2 text-xs text-slate-400">
                      ...
                      <span className="text-slate-600">•</span>
                      <button className="text-red-300 hover:text-red-200" onClick={() => void handleDelete('area', area.id)}>
                        Delete
                      </button>
                    </div>
                  </article>
                ))}
                {areas.length === 0 && <p className="text-center text-slate-500">No areas yet.</p>}
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-slate-950/40">
              <header className="flex items-center justify-between border-b border-white/5 px-4 py-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Sub areas</p>
                  <p className="text-2xl font-semibold text-white">{subAreas.length}</p>
                </div>
              </header>
              <div className="max-h-105 space-y-2 overflow-auto p-4 text-sm text-slate-200">
                {subAreas.map((subArea) => {
                  const parent = areas.find((area) => area.id === subArea.areaId)
                  return (
                    <article key={subArea.id} className="rounded-2xl border border-white/5 p-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-base font-semibold text-white">{subArea.name}</p>
                        ...
                        <span
                          className={`rounded-full px-3 py-1 text-xs font-semibold ${
                            subArea.active ? 'bg-emerald-500/20 text-emerald-200' : 'bg-slate-600/40 text-slate-300'
                          }`}
                        >
                          {subArea.active ? 'Active' : 'Inactive'}
                        </span>
                      </div>
                      <p className="mt-2 text-xs text-slate-400">{subArea.description || 'No description'}</p>
                      <div className="mt-3 flex items-center gap-2 text-xs text-slate-400">
                        ...
                        <span className="text-slate-600">•</span>
                        <button className="text-red-300 hover:text-red-200" onClick={() => void handleDelete('subArea', subArea.id)}>
                          Delete
                        </button>
                      </div>
                    </article>
                ...
                {subAreas.length === 0 && <p className="text-center text-slate-500">No sub areas yet.</p>}
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-slate-950/40">
              <header className="flex items-center justify-between border-b border-white/5 px-4 py-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Branches</p>
                  <p className="text-2xl font-semibold text-white">{branches.length}</p>
                </div>
              </header>
              <div className="max-h-105 space-y-2 overflow-auto p-4 text-sm text-slate-200">
                {branches.map((branch) => {
                  const parentArea = areas.find((area) => area.id === branch.areaId)
                  const parentSubArea = branch.subAreaId
                    ? subAreas.find((subArea) => subArea.id === branch.subAreaId)
                    : undefined
                  return (
                    <article key={branch.id} className="rounded-2xl border border-white/5 p-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-base font-semibold text-white">{branch.name}</p>
                          <p className="text-xs text-slate-400">
                            {parentArea?.name ?? '—'} {parentSubArea ? `· ${parentSubArea.name}` : ''}
                          </p>
                        </div>
                        <span
                          className={`rounded-full px-3 py-1 text-xs font-semibold ${
                            branch.active ? 'bg-emerald-500/20 text-emerald-200' : 'bg-slate-600/40 text-slate-300'
                          }`}
                        >
                          {branch.active ? 'Active' : 'Inactive'}
                        </span>
                      </div>
                      <p className="mt-2 text-xs text-slate-400">{branch.description || 'No description'}</p>
                      <div className="mt-3 flex items-center gap-2 text-xs text-slate-400">
                        ...
                        <span className="text-slate-600">•</span>
                        <button className="text-red-300 hover:text-red-200" onClick={() => void handleDelete('branch', branch.id)}>
                          Delete
                        </button>
                      </div>
                    </article>
                ...
                {branches.length === 0 && <p className="text-center text-slate-500">No branches yet.</p>}
              </div>
            </div>
          </div>
        </section>
