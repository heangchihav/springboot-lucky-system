...
const COMPANY_DETAILS: Record<CompanyKey, CompanyDetail> = {
  'J&T': {
    ...
    highestPrice: 180000,
    lowestPrice: 4000,
    strengths: ['Fast delivery nationwide', 'Strong tracking system', 'Wide branch coverage'],
    weaknesses: ['Customer service response can be slow', 'Occasional parcel delays during peak time'],
...
const buildChartData = (areaId: number | 'all', provinceName: string | 'all') => {
  const selectedAreas = areaId === 'all' ? AREAS : AREAS.filter((area) => area.id === areaId)
...
const buildDetails = (areaId: number | 'all', provinceName: string | 'all') => {
  ...
  const items: { area: string; province: string; companies: Record<CompanyKey, number>; total: number }[] = []
  ...
  selectedAreas.forEach((area) => {
    area.provinces.forEach((province) => {
      ...
      const total = companyKeys.reduce((sum, company) => sum + province.companies[company], 0)
...
export default function CompetitorsDashboardPage() {
  ...
  const availableProvinces = useMemo(() => {
    if (selectedArea === 'all') {
      return AREAS.flatMap((area) => area.provinces.map((province) => province.name))
    }
  ...
  const detailRows = useMemo(() => buildDetails(selectedArea, selectedProvince), [selectedArea, selectedProvince])
    ...
    <MarketingServiceGuard>
      <div className="space-y-8">
        <header className="space-y-2">
          <p className="text-xs uppercase tracking-[0.4em] text-amber-300/70">Marketing ◦ Company landscape</p>
          <h1 className="text-3xl font-semibold text-white">Competitors dashboard</h1>
          <p className="text-sm text-slate-300">Filter by area/province to explore company presence and total branch density across Cambodia.</p>
        </header>

        <section className="rounded-3xl border border-white/10 bg-white/5 p-5 text-white">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Total branches</p>
              <p className="mt-3 text-4xl font-semibold">{totalBranches}</p>
              <p className="text-sm text-slate-300">Based on filter selection across all companies</p>
            </div>
            <div className="flex flex-col gap-4 text-sm lg:flex-row lg:items-center lg:gap-6">
              <div className="flex flex-col">
                <label className="text-[0.6rem] uppercase tracking-[0.3em] text-slate-300">Area</label>
                <select
              ...
              <div className="flex flex-col">
                <label className="text-[0.6rem] uppercase tracking-[0.3em] text-slate-300">Province</label>
                <select
                  className="mt-2 w-44 rounded-2xl border border-white/20 bg-slate-900/60 px-4 py-2 text-sm text-white focus:border-amber-400/60 focus:outline-none"
                  value={selectedProvince}
                  onChange={(event) => setSelectedProvince(event.target.value as string | 'all')}
        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-amber-300/70">Company density</p>
              <h2 className="text-xl font-semibold text-white">Company comparison</h2>
            </div>
            <div className="flex flex-wrap gap-3 text-xs text-slate-300">
              {companyKeys.map((company) => (
                <span key={company} className="flex items-center gap-2 rounded-full border border-white/10 px-3 py-1">
                  <span className="h-2 w-2 rounded-full" style={{ backgroundColor: companyPalette[company] }} />
                  {company}
                </span>
              ))}
            </div>
          ...
          <div className="mt-6 h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 32, left: 16, right: 16, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#475569" opacity={0.2} />
                <XAxis dataKey="company" stroke="#94a3b8" />
                <YAxis stroke="#94a3b8" domain={[0, (dataMax: number) => (dataMax === 0 ? 10 : dataMax * 1.2)]} />
                <Tooltip
                ...
                <Bar dataKey="total" barSize={70} radius={[12, 12, 0, 0]}>
                  ...
                  />
                  {chartData.map((entry) => (
                    <Cell key={entry.company} fill={companyPalette[entry.company as CompanyKey]} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-amber-300/70">Detail breakdown</p>
              <h2 className="text-xl font-semibold text-white">All branches by company</h2>
            </div>
            <span className="rounded-full border border-white/10 px-3 py-1 text-xs text-slate-200">{detailRows.length} rows</span>
          </div>
              ...
              <tbody className="divide-y divide-white/5">
                {detailRows.map((row) => (
                  <tr key={`${row.area}-${row.province}`}>
                    ...
                    {companyKeys.map((company) => (
                      <td key={company} className="py-3 pr-6 font-semibold text-white" style={{ color: companyPalette[company] }}>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-amber-300/70">Company intel</p>
              <h2 className="text-xl font-semibold text-white">Competitive pricing</h2>
            </div>
            <p className="text-sm text-slate-300">Hover for long remarks to understand when each partner is strongest.</p>
          </div>
          <div className="mt-6 overflow-x-auto">
            <table className="min-w-full text-left text-sm text-slate-200">
              <thead>
                    ...
                    <tr key={company}>
                      <td className="py-3 pr-6">
                        <div className="flex items-center gap-3">
                          <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: companyPalette[company] }} />
                          <span className="font-semibold">{detail.name}</span>
                        </div>
                      </td>
                      <td className="py-3 pr-6">
                        <div className="min-w-48 whitespace-nowrap">
                          <span className="font-semibold text-white">{formatPrice(detail.lowestPrice)}</span>
                          <span className="mx-2 text-slate-400">–</span>
                          <span className="font-semibold text-white">{formatPrice(detail.highestPrice)}</span>
                        </div>
                      </td>
