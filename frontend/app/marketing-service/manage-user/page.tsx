...
import {
  ...
  VipMember,
  VipMemberPayload,
} from '../services/vipMemberService'
...
export default function MarketingVipManageUserPage() {
  ...
  useEffect(() => {
    ...
    const loadMembers = async () => {
      ...
      try {
        const params: { areaId?: number; subAreaId?: number; branchId?: number } = {}
  ...
  useEffect(() => {
    if (form.branchId && !filteredBranchesForForm.some((branch) => branch.id === form.branchId)) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const payload: VipMemberPayload = {
        ...
        memberDeletedAt: form.memberDeletedAt ? form.memberDeletedAt : undefined,
        createRemark: form.createRemark?.trim() ? form.createRemark.trim() : undefined,
        deleteRemark: form.deleteRemark?.trim() ? form.deleteRemark.trim() : undefined,
      }
      ...
      showToast(error instanceof Error ? error.message : 'Failed to save member', 'error')
    } finally {
      ...
      <div className="space-y-8">
        <header className="space-y-2">
          ...
          <p className="text-sm text-slate-300">
            Create, update, or archive members across areas, sub-areas, and branches. Data is live
            with the marketing-service backend.
          </p>
        </header>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 p-5">
          <div className="grid gap-4 lg:grid-cols-3">
              ...
              <div className="mt-3 space-y-3 text-xs text-slate-300">
                <div className="flex flex-col">
                  ...
                  <select
                    ...
                    onChange={(event) => handleFilterAreaChange(event.target.value)}
                  >
                ...
                {filters.areaId !== 'all' && (
                  <div className="flex flex-col">
                    ...
                    <select
                      ...
                      onChange={(event) => handleFilterSubAreaChange(event.target.value)}
                    >
                ...
                {filters.areaId !== 'all' && (
                  <div className="flex flex-col">
                    ...
                    <select
                      ...
                      onChange={(event) => handleFilterBranchChange(event.target.value)}
                    >
            ...
            <div className="rounded-2xl border border-white/10 bg-emerald-500/10 p-4">
              <p className="text-xs uppercase tracking-[0.3em] text-emerald-200/80">Active</p>
              <p className="mt-2 text-3xl font-semibold text-white">{totalActiveMembers}</p>
              <p className="text-sm text-emerald-100/80">Members currently active</p>
            </div>

            <div className="rounded-2xl border border-white/10 bg-rose-500/10 p-4">
              <p className="text-xs uppercase tracking-[0.3em] text-rose-200/80">Archived</p>
              <p className="mt-2 text-3xl font-semibold text-white">{totalArchivedMembers}</p>
              <p className="text-sm text-rose-100/80">Members removed or inactive</p>
            </div>
          </div>
        </section>
            ...
            <form className="flex-1 space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="flex flex-col text-xs text-slate-300 md:col-span-2">
                  ...
                  <select
                    value={form.areaId ?? ''}
                    onChange={(event) => handleFormAreaChange(event.target.value)}
                    className="mt-1 rounded-xl border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-white focus:border-amber-400/60 focus:outline-none"
                  >
                ...
                {filteredSubAreasForForm.length > 0 && (
                  <div className="flex flex-col text-xs text-slate-300">
                    ...
                    <select
                      ...
                      onChange={(event) => handleFormSubAreaChange(event.target.value)}
                      className="mt-1 rounded-xl border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-white focus:border-amber-400/60 focus:outline-none"
                    >
                      <option value="all">All sub areas</option>
                ...
                <div className="flex flex-col text-xs text-slate-300">
                  ...
                  <select
                    value={form.branchId ?? ''}
                    onChange={(event) => handleFormBranchChange(event.target.value)}
                ...
                <div className="flex flex-col text-xs text-slate-300">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setForm((prev) => ({ ...prev, memberCreatedAt: event.target.value }))
                    }
                    className="mt-1 rounded-xl border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-white focus:border-amber-400/60 focus:outline-none"
                  />
                </div>
                ...
                <div className="flex flex-col text-xs text-slate-300">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setForm((prev) => ({ ...prev, memberDeletedAt: event.target.value }))
                    }
                    className="mt-1 rounded-xl border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-white focus:border-amber-400/60 focus:outline-none"
                  />
                </div>
                ...
                <div className="flex flex-col text-xs text-slate-300 md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      setForm((prev) => ({ ...prev, createRemark: event.target.value }))
                    }
                ...
                <div className="flex flex-col text-xs text-slate-300 md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      setForm((prev) => ({ ...prev, deleteRemark: event.target.value }))
                    }
                    className="mt-1 rounded-2xl border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-white focus:border-amber-400/60 focus:outline-none"
                  />
                </div>
              ...
              >
                {formSubmitting ? 'Saving…' : editingMember ? 'Update member' : 'Create member'}
              </button>
            </form>
                        ...
                        >
                          ...
                          <td className="px-4 py-4 text-sm text-slate-200">
                            ...
                            <p className="text-xs text-slate-400">
                              {member.subAreaName ? `${member.subAreaName} · ` : ''}
                              {member.branchName ?? '—'}
                            </p>
                          </td>
                          <td className="px-4 py-4 text-xs text-slate-300">
                            <p>
                              Joined <span className="text-white">{member.memberCreatedAt}</span>
                            </p>
