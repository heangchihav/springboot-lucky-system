...
export default function ManageUserPage() {
  ...
  const [editingUserBranchCurrentId, setEditingUserBranchCurrentId] = useState<
    number | null
  >(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedBranchId) {
        await areaBranchService.assignUserToBranch(
          newUser.id,
          selectedBranchId,
        );
      }
  ...
  const loadBranches = async () => {
    try {
      ...
      if (currentUserId) {
        try {
          const assignments =
            await areaBranchService.getUserBranchesByUser(currentUserId);
          ...
          console.error(
            "Error loading current user branch assignments:",
            assignmentError,
          );
        }
      }
    ...
    try {
      ...
      if (branchChanged) {
        if (editingUserBranchCurrentId) {
          await areaBranchService.removeUserFromBranch(
            editingUser.id,
            editingUserBranchCurrentId,
          );
        }
        if (editBranchId) {
          await areaBranchService.assignUserToBranch(
