...
class CallPermissionsService {
    ...
    async getPermissionGroups(): Promise<PermissionGroup[]> {
        ...
        return Object.entries(grouped)
            .sort(([a], [b]) => {
                // Sort groups: Dashboard, Areas, Sub-Areas, Branches, User Branch Assignments, Call Reports, Call Status, Role Management, Permissions Management, User Management, Other
                const order = [
                    "Dashboard",
                    "Areas",
                    "Sub-Areas",
    ...
    private getGroupDescription(groupName: string): string {
        const descriptions: Record<string, string> = {
            ...
            Areas: "Management of call service areas",
            "Sub-Areas": "Management of call service sub-areas",
            Branches: "Management of call service branches",
            "User Branch Assignments": "User branch assignment management",
            "Call Reports": "Call report management and tracking",
            "Call Status": "Call status configuration and management",
            "Role Management": "Role administration and assignment",
            "Permissions Management": "Permission administration and configuration",
            "User Management": "User account management and administration",
    ...
    async assignUsersToRole(roleId: number, userIds: number[]): Promise<void> {
        const response = await apiFetch(
            `/api/calls/roles/${roleId}/assign-users`,
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
    ...
    async removeUsersFromRole(roleId: number, userIds: number[]): Promise<void> {
        const response = await apiFetch(
            `/api/calls/roles/${roleId}/remove-users`,
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
    ...
    async checkUserPermission(
        ...
        try {
            const response = await apiFetch(
                `/api/calls/permissions/user/${userId}/check/${permissionCode}`,
                {
