...
# ============================================================================
# PostgreSQL Backup Script for Docker Compose
# ============================================================================
# This script creates timestamped backups of all PostgreSQL databases
...
echo -e "${YELLOW}=== PostgreSQL Backup Started ===${NC}"
...
for db in "${DATABASES[@]}"; do
    ...
    if docker exec "${CONTAINER_NAME}" pg_dump -U "${POSTGRES_USER}" "${db}" | gzip > "${BACKUP_FILE}"; then
...
if docker exec "${CONTAINER_NAME}" pg_dumpall -U "${POSTGRES_USER}" | gzip > "${ALL_BACKUP_FILE}"; then
